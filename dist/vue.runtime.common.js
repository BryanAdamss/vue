...
// 访问target.key实际访问的是target.[sourceKey][key]
function proxy(target, sourceKey, key) {
  sharedPropertyDefinition.get = function proxyGetter() {
    return this[sourceKey][key] // this指向target
  }
...
//   }
// }
// 形如上面的computed，target是{name:xxx},key是fullName，userDef是function(){return this.firstName + this.lastName}
function defineComputed(target, key, userDef) {
  // 服务端渲染不缓存
  var shouldCache = !isServerRendering()

  // computed支持设置函数及对象(设置了set、get的对象)
  if (typeof userDef === 'function') {
    // 将get设置为userDef
    sharedPropertyDefinition.get = shouldCache
      ? createComputedGetter(key) // 从缓存中，拿到computedWatcher的值
      : userDef // 否则直接调用，取值

    // 函数不需要设置set
    sharedPropertyDefinition.set = noop
  } else {
    // userDef是带有get、set的对象
    sharedPropertyDefinition.get = userDef.get
      ? shouldCache && userDef.cache !== false
        ? createComputedGetter(key) // 从缓存中，拿到computedWatcher的值
        : userDef.get // 否则直接调用userDef.get取值
      : noop
    // userDef传入了set，则设置
...
function initExtend(Vue) {
   ...
   */
  // Vue在内的实例构造函数，都有一个唯一constructor id；方便缓存
  Vue.cid = 0
  var cid = 1
   ...
   * Class inheritance
   */
  // Vue.extend主要使用基础 Vue 构造器，创建一个“子类”。(Vue中创建组件内部也是使用的Vue.extend)；参数是一个包含组件选项的对象
  Vue.extend = function(extendOptions) {
    ...
    Sub.cid = cid++
    // Sub继承父类的options；将父类选项和传入的选项合并，并赋值给子类
    ...
    // Sub.options中有props和computed则初始化它们
    if (Sub.options.props) {
      // 将props中的key代理到Sub.prototype._props上
      // 这样vm.test(vm.props.test)实际访问到的是vm.prototype._props.test
      initProps$1(Sub)
    }
    ...
    // 拷贝父类的['component', 'directive', 'filter']这些asset到子类
    ASSET_TYPES.forEach(function(type) {
      Sub[type] = Super[type]
    })

    // enable recursive self-lookup
    // 启用递归自查找，即通过Sub.options.components[name]就能找到Sub构造函数
    if (name) {
    ...
    // been updated.
    // 在子类上保存父类options、子类自身options、合并后的options的拷贝
